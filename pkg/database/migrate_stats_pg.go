...
func (db *DB) MigrateStatsPG() error {
	return db.migratePG(
		...
		migrateStatsIndexesPG,
	)
}

func migrateStatsIndexesPG(ctx context.Context, tx pgx.Tx) error {
	_, err := tx.Exec(
		ctx,
		`
		`)
...
func migrateStats000SchemaPG(ctx context.Context, tx pgx.Tx) error {
	_, err := tx.Exec(
		...
		`
			...
			CREATE TABLE IF NOT EXISTS stats.client_names (
			...
			CREATE TABLE IF NOT EXISTS stats.client_user_data (
				client_user_data_id	SERIAL	PRIMARY KEY,
				client_user_data	TEXT	NOT NULL,

				CONSTRAINT client_user_data_unique UNIQUE (client_user_data)
			);

			CREATE TABLE IF NOT EXISTS stats.client_versions (
				client_version_id	SERIAL	PRIMARY KEY,
				client_version		TEXT	NOT NULL,

				CONSTRAINT client_version_unique UNIQUE (client_version)
			);

			CREATE TABLE IF NOT EXISTS stats.crawled_nodes (
				timestamp			TIMESTAMPTZ	NOT NULL,
				client_name_id		INTEGER		DEFAULT NULL REFERENCES stats.client_names(client_name_id),
				client_user_data_id	INTEGER		DEFAULT NULL REFERENCES stats.client_user_data(client_user_data_id),
				client_version_id	INTEGER		DEFAULT NULL REFERENCES stats.client_versions(client_version_id),
				client_os			SMALLINT	NOT NULL,
				client_arch			SMALLINT	NOT NULL,
