...
var v4Bootnodes = func() []*enode.Node {
	bootnodes := make([]*enode.Node, len(params.MainnetBootnodes))

	for i, record := range params.MainnetBootnodes {
		bootnodes[i] = enode.MustParseV4(record)
	}

	return bootnodes
}()

// Copied from prysm:config/params/mainnet_config.go
var v5BootstrapRecords = []string{
...
var v5Bootnodes = func() []*enode.Node {
	bootnodes := make([]*enode.Node, len(v5BootstrapRecords))

	for i, record := range v5BootstrapRecords {
		bootnodes[i] = enode.MustParse(record)
	}
